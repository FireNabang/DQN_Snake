    ...
    def move(self):
        ...
        if isLiving == 1:
            ...
            if self.playType == 2:
                ...
                hx = self.position[0][1]
                for c in qmap[hy][hx]:
                    if c["length"] == len(self.position) and checkSameList(c["head"],
                                                                           self.position[0]) and checkSameList(
                            c["tail"], self.position[len(self.position) - 1]) and checkSameList(c["apple"], [fy,
                                                                                                             fx]) and checkSameList(
                            c["direction"], self.dir):
                        c["value"][self.dir] = -1
            self.updateQ_Value()
