...
class DQNAgent:
    ...
    def create_model(self):
        ...
        model.add_layer(DenseLayer(self.field_height * self.field_width))
        model.add_layer(DenseLayer(32, activations='relu'))
        model.add_layer(DenseLayer(16, activations='relu'))
        model.add_layer(DenseLayer(4, activations='relu'))
        return model
    ...
    def get_q_values(self, x):
        return self.model.predict(np.array([np.reshape(x, (self.field_height * self.field_width, 1))]))
    ...
    def train(self):
        ...
        current_input = np.array([np.reshape(sample[0], (self.field_height * self.field_width, 1)) for sample in samples])
        current_q_values = self.model.predict(current_input)

        next_input = np.array([np.reshape(sample[3], (self.field_height * self.field_width, 1)) for sample in samples])
        next_q_values = self.model.predict(next_input)

        # update q values
        for i, (_, action, reward, _, live) in enumerate(samples):
            if live == 0:
                next_q_value = reward
            else:
